...
class CouponAlt extends JsonEntity implements \JsonSerializable
{
    public int $id;
    ...
    public static function initCouponAlt(string $code, string $description, int $value): CouponAlt
    {
        $coupon = new CouponAlt();

        $coupon->code = $code;
        $coupon->shortDescription = $description;
        $coupon->value = intval($value) . '.00';
        $coupon->currency = Currency::initUSD()->id;
